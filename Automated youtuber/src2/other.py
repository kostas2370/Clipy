...
def check_file(file_name):
        final_name=file_name+".mp4"
        path=get_Destination(0)
        final_path=path+"/"+final_name
        files_to_delete_path=get_Destination(2)+"/"+final_name
...
def get_down_history(what,x):
    ...
    if(x=="*"):
             try:
                    c.execute("SELECT {} FROM youtube_clips".format (what))
...
def check_if_db(link):
    conn=sqlite3.connect("db/database.db")
    c=conn.cursor()
    c.execute("SELECT * FROM youtube_clips WHERE video_link ='{}'".format(link))
    if len(c.fetchall())>=1:
        x=  True    
    else:
        x=  False
...
def set_intro(path,filename):
    ...
    try:
        x=path+"/"+filename
        destination=destination_to_go+"/"+"intro.mp4"     
        os.system("ffmpeg -i \"{}\" -vf \"scale=1920:1080\" \"{}\"".format(x,destination))
    except:
...
def set_outro(path,filename):
    ...
    try:
        x=path+"/"+filename
        destination=destination_to_go+"/"+"outro.mp4"        
        os.system("ffmpeg -i \"{}\" -vf \"scale=1920:1080\" \"{}\"".format(x,destination))
    except:
        print("We can't find this video")
        return False
...
def get_creator(file_name):
    conn=sqlite3.connect("db/database.db")
    c=conn.cursor()
    c.execute("SELECT channel_name FROM youtube_clips WHERE file_name= '{}'".format(file_name))
    lista=c.fetchall()
    creator=""
    if (len(lista)>0):
        ...
        c.execute("SELECT publisher FROM cutted_clip WHERE clip_name= '{}'".format(file_name[0:len(file_name)-3]))    
        lista=c.fetchall()
        for x in lista:
            creator=x[0]
